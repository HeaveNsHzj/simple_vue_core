...
/***/ function(module, exports, __webpack_require__) {
	...
	var _diffMatchPatch = __webpack_require__(1);
	...
	Editor.prototype.showDiffWithFile = function (filename) {
	  ...
	  fetchFile(filename).then(function (content) {
	    var dmp = new _diffMatchPatch.diff_match_patch();
	    var diffs = dmp.diff_lineMode_(content, _this2.editor.getValue());
	    dmp.diff_cleanupSemantic(diffs);
	    _this2.highlightDiff(diffs);
	  });
	};
	...
	Editor.prototype.hideDiff = function () {
	  ...
	  if (this.ranges && this.ranges.length) {
	    this.ranges.forEach(function (range) {
	      _this3.session.removeMarker(range.id);
	    });
	...
	function initCodeFor(container, files) {
	  ...
	  return Array.from(files).map(function (obj) {
	    ...
	    li.innerHTML = '<div class="file-title">\
	        ...
	        <div class="fr">' + (prevFile ? '<label><input type="checkbox" class="show-diff"/>高亮新增行</label>' : '') + '<i class="font-zoom-in">+</i>\
	    ...
	    fetchFile(filename).then(function (code) {
	...
	(0, _console.createLogger)(document.querySelector('#console-container'));
...
/***/ function(module, exports) {
	...
	function createLogger(el) {
	  var tagName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'li';
	  ...
	  window.console.log = function () {
	    ...
	    try {
	      ...
	      for (var i = 0, len = args.length; i < len; i++) {
	        text.push(_typeof(args[i]) == 'object' ? JSON.stringify(args[i]) : String(args[i]));
	      }
